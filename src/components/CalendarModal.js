...
function CalendarModal({ calendarVisible, setCalendarVisible, range, setRange, onConfirm , getOrders}) {
...
if(type === "START_DATE"){
  console.log(date )
  setStartDate( JSON.stringify(date))
}
if(type==="END_DATE"){
      ...
      >
        ...
        />
            ...
            <View className="flex-row gap-3 justify-center mt-4 px-5">
              ...
              >
                <Text className="font-semibold text-[#444262] text-center">Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
            className="bg-[#444262] flex-1 py-2 rounded-lg"
              onPress={()=>getOrders(startDate , endDate)}
            >
