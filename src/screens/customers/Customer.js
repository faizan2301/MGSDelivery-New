...
const Customers = ({ route, navigation }) => {
    ...
    // console.log(array)
    const [allCustomer, { data, error, isError, isSuccess, isLoading }] = useCustomersMutation();
    ...
    useEffect(() => {
        if (isError) {
            if (error.data) {
                ...
                showMessage({ message: "Somrthing went wrong while get customers", type: " danger" })
            }
        }
    }, [isError])
        ...
        return ({ item }) => {
            ...
            return (
                <>
                    <Pressable
                        onPress={() => {
                          navigation.navigate(navigationStrings.ADMINMYCUSTOMERDETAILS , {item})
