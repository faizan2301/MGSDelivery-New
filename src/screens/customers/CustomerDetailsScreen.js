...
const CustomerDetailsScreen = ({ route, navigation }) => {
    ...
    const [customerOrders, { data, isSuccess, isError, error, isLoading }] = useCustomerOrdersMutation()
    useEffect(() => {
    ...
    useEffect(() => {
        if (isError) {
            if (error.data) {
                ...
                showMessage({ message: "Something went wrong while fetching orders", type: 'danger' })
            }
        }
    }, [isError])
    ...
    const renderItem = useMemo(() => {
        return ({ item, index }) => {
            return (
                <>
                    <TouchableOpacity
                        onPress={() => navigation.navigate(navigationStrings.ADMINMYCUSTOMERORDERDETAILS, { item })}
                    ...
                    >
                        ...
                        <View className="flex-row justify-between  gap-2 flex-wrap px-2">
                            ...
                            <View className=" items-center justify-center">
                                {item?.orderId?.paid ? <View className="bg-green-600 w-4 h-4 rounded-full">

                                </View> :
                                    <View className="bg-red-600 w-4 h-4 rounded-full">
                                    </View>}
                            </View>
                        </View>
                        ...
                        <View className="flex-row  gap-2 flex-wrap">
                            ...
                            >
                                ...
                                <Text className="pl-3">{item?.orderId?.createdAt}</Text>
                            </View>
    ...
    return (
        <View className="px-2 py-2">
            <View className=" px-5 overflow-auto py-3 rounded-lg shadow-lg shadow-black bg-white">
                ...
                <View className="px-2 py-2">
                    <TouchableOpacity onPress={() => setModalVisible(true)} className="border py-2 rounded-md mt-2 text-center bg-[#444262]">
            ...
            <CollectCreditAmount collectCreditAmount={collectCreditAmount} modalVisible={modalVisible} item={item} setModalVisible={setModalVisible} />
        </View>
    )
}
