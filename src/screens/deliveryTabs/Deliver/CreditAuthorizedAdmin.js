...
const CreditAuthorizedAdmin = ({ route, navigation }) => {
  ...
  const [
    sendRequiest,
    {
      ...
      isSuccess: sendRequestIsSuccess,
      error: sendRequestError,
      isError: sendRequestIsError,
  ...
  const getData = async () => {
    var res = await getCrediAuthorizedAdmin({ body: {}, token });
    console.log(res);
  };
  useEffect(() => {
    if (isSuccess) {
  ...
  useEffect(() => {
    if (isError) {
      if (error.data) {
        ...
        showMessage({
          message: "Somthing went wrong while fetching admin data",
          type: "danger",
        });
      }
    }
  ...
  useEffect(() => {
    if (sendRequestIsSuccess) {
      showMessage({
        message: "Credit Approval requiest send successfullys",
        type: "success",
      });
      navigation.goBack();
    }
  ...
  useEffect(() => {
    if (sendRequestIsError) {
      if (sendRequestError.data) {
        ...
        showMessage({
          message: "Something went wrong while sending requiest",
          type: "danger",
        });
      }
    }
    ...
    <View className="bg-slate-100 flex-1 px-3">
      ...
      {isLoading && <LoadingModal loading={isLoading} />}
      {
        <SendCreditRequeist
          ...
          sendRequeist={sendCreditRequiest}
        />
      }
    </View>
  );
